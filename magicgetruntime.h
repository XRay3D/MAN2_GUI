...
{
    QVariant ret;
    return ((I == idx ? ret = pfr::get<I>(pod) : ret), ..., ret);
}
...
struct getter final {
    template <typename T>
    auto operator()(T&& pod) { return (pfr::get<I>(pod)); }
};
...
struct array {
    static constexpr auto size = sizeof(getter<Rt, T, 0>);
    static inline char data[size * sizeof...(Is)] = {};
    static inline getter_i<Rt, T>* funcs[] = { new (data + Is * size) getter<Rt, T, Is>... };
};

...
void Graduation::on_pbStartGrad_clicked()
{
    ...
    if (rbVoltage->isChecked()) {
        if (m_channel == 9) {
            ...
            QMessageBox::information(this, "", "Подайте на вход ~100В");
            qApp->processEvents();

            for (int i = 0, counter = 1; i < count; ++i, ++counter) {
                for (int j = 0; j < 100; ++j, ++counter) {
                    measure[Meas1] += MI::man()->GetMeasuredValueRMS();
                    thread()->msleep(10);
            ...
            for (int i = 0, counter = 1; i < count; ++i, ++counter) {
                for (int j = 0; j < 100; ++j, ++counter) {
                    measure[Meas2] += MI::man()->GetMeasuredValueRMS();
                    thread()->msleep(10);
            ...
            measure[Meas2] /= count * 100;
            reference[Meas2] /= count;

            m_chbxs[4]->setChecked(true);

            GradCoeff.AdcCh1Offset = ((reference[Meas2] - reference[Meas1]) / (measure[Meas2] - measure[Meas1]));
            GradCoeff.AdcCh2Offset = reference[Meas1] - (measure[Meas1] * GradCoeff.AdcCh1Offset);

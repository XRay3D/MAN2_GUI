/****************************************************************************
** Meta object code from reading C++ file 'man2.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.12.9)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../ElektrosilaDev/hw/man2.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'man2.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.12.9. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_MAN2_t {
    QByteArrayData data[15];
    char stringdata0[129];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_MAN2_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_MAN2_t qt_meta_stringdata_MAN2 = {
    {
QT_MOC_LITERAL(0, 0, 4), // "MAN2"
QT_MOC_LITERAL(1, 5, 15), // "detectedAddress"
QT_MOC_LITERAL(2, 21, 0), // ""
QT_MOC_LITERAL(3, 22, 4), // "mode"
QT_MOC_LITERAL(4, 27, 4), // "Open"
QT_MOC_LITERAL(5, 32, 5), // "Close"
QT_MOC_LITERAL(6, 38, 5), // "Write"
QT_MOC_LITERAL(7, 44, 4), // "data"
QT_MOC_LITERAL(8, 49, 13), // "measCompleted"
QT_MOC_LITERAL(9, 63, 13), // "MeasuredValue"
QT_MOC_LITERAL(10, 77, 20), // "GetMeasuredValueSlot"
QT_MOC_LITERAL(11, 98, 9), // "ValueType"
QT_MOC_LITERAL(12, 108, 4), // "type"
QT_MOC_LITERAL(13, 113, 7), // "uint8_t"
QT_MOC_LITERAL(14, 121, 7) // "channel"

    },
    "MAN2\0detectedAddress\0\0mode\0Open\0Close\0"
    "Write\0data\0measCompleted\0MeasuredValue\0"
    "GetMeasuredValueSlot\0ValueType\0type\0"
    "uint8_t\0channel"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_MAN2[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       5,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   54,    2, 0x06 /* Public */,
       4,    1,   57,    2, 0x06 /* Public */,
       5,    0,   60,    2, 0x06 /* Public */,
       6,    1,   61,    2, 0x06 /* Public */,
       8,    1,   64,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
      10,    2,   67,    2, 0x0a /* Public */,
      10,    1,   72,    2, 0x2a /* Public | MethodCloned */,
      10,    0,   75,    2, 0x2a /* Public | MethodCloned */,

 // signals: parameters
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void,
    QMetaType::Void, QMetaType::QByteArray,    7,
    QMetaType::Void, 0x80000000 | 9,    2,

 // slots: parameters
    QMetaType::Void, 0x80000000 | 11, 0x80000000 | 13,   12,   14,
    QMetaType::Void, 0x80000000 | 11,   12,
    QMetaType::Void,

       0        // eod
};

void MAN2::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<MAN2 *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->detectedAddress((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->Open((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 2: _t->Close(); break;
        case 3: _t->Write((*reinterpret_cast< const QByteArray(*)>(_a[1]))); break;
        case 4: _t->measCompleted((*reinterpret_cast< const MeasuredValue(*)>(_a[1]))); break;
        case 5: _t->GetMeasuredValueSlot((*reinterpret_cast< ValueType(*)>(_a[1])),(*reinterpret_cast< uint8_t(*)>(_a[2]))); break;
        case 6: _t->GetMeasuredValueSlot((*reinterpret_cast< ValueType(*)>(_a[1]))); break;
        case 7: _t->GetMeasuredValueSlot(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            using _t = void (MAN2::*)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&MAN2::detectedAddress)) {
                *result = 0;
                return;
            }
        }
        {
            using _t = void (MAN2::*)(int );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&MAN2::Open)) {
                *result = 1;
                return;
            }
        }
        {
            using _t = void (MAN2::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&MAN2::Close)) {
                *result = 2;
                return;
            }
        }
        {
            using _t = void (MAN2::*)(const QByteArray & );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&MAN2::Write)) {
                *result = 3;
                return;
            }
        }
        {
            using _t = void (MAN2::*)(const MeasuredValue & );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&MAN2::measCompleted)) {
                *result = 4;
                return;
            }
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject MAN2::staticMetaObject = { {
    &QObject::staticMetaObject,
    qt_meta_stringdata_MAN2.data,
    qt_meta_data_MAN2,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *MAN2::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *MAN2::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_MAN2.stringdata0))
        return static_cast<void*>(this);
    if (!strcmp(_clname, "CommonInterfaces"))
        return static_cast< CommonInterfaces*>(this);
    return QObject::qt_metacast(_clname);
}

int MAN2::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 8)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 8;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 8)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void MAN2::detectedAddress(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void MAN2::Open(int _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void MAN2::Close()
{
    QMetaObject::activate(this, &staticMetaObject, 2, nullptr);
}

// SIGNAL 3
void MAN2::Write(const QByteArray & _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 3, _a);
}

// SIGNAL 4
void MAN2::measCompleted(const MeasuredValue & _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 4, _a);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
